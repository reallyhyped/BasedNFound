...
async def use_id(user_id: int, database=Depends(get_database)):
    find_query = "SELECT * FROM bnf_user WHERE id = :user_id"
    found_user = await database.fetch_one(find_query, {"user_id": user_id})
...
async def update_user(
    username: str, update_data: UserUpdate, database=Depends(get_database)
):
    find_query = "SELECT * FROM bnf_user WHERE username = :username"
    found_user = await database.fetch_one(find_query, {"username": username})
